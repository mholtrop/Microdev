...
#define USER_LED GPIO_NUM_21 // GPIO_NUM_21
...
void app_main() {
   ...
   // The NVS partition is used by the WiFi library.
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        // esp_restart();  // Hard restart will reset boot count.
    } else {
    ...
    // // Setup Time. See: https://esp32tutorials.com/esp32-sntp-esp-idf-synchronize-time-ntp/
    // initialize_sntp();
    // time_t now = 0;
    ...
    esp_deep_sleep(57000000ULL); /* One minute - 3s of deep sleep, then it reboots. */
}