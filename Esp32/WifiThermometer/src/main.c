...
//
#define WIFI_SSID "Porenquanto" // "Hobbit"
#define WIFI_PASS "provisorio" // "magdalena"
// Password:
// MQTT Broker:
#define MQTT_broker "mqtt://192.168.0.15"
// MQTT User:
// MQTT Password:
//
///////////////////////////////////////////////////////////////////////////////////////
...
//ADC_ATTEN_DB_0 = No input attenuation, ADC can measure up to Vref which is approx 1100mV (factory test value burnt into eFuse)
...
//ADC_ATTEN_DB_12 = input attenuated extending the range of measurement by about 12 dB (3.55 x)
   ...
   if (event_base == WIFI_EVENT){
      switch(event_id){
         case WIFI_EVENT_STA_START:
            printf("Wifi Event Start, try esp_wifi_connect()\n");
            fflush(stdout);
...
void wifi_start(void){
   ...
   wifi_config_t wifi_config = {
         .sta = {
               .ssid = WIFI_SSID,
               .password = WIFI_PASS,
               .threshold.authmode = WIFI_AUTH_WPA_WPA2_PSK  ,  //WIFI_AUTH_WPA2_PSK,
               .pmf_cfg = {
                     .capable = true,
                     .required = false},
         },
...
void app_main() {
   ...
   printf("Here again is the Boot num: %d\n",boot_count);
   fflush(stdout);
   ...
   esp_log_level_set("mqtt_client",ESP_LOG_INFO);     // Silence the MQTT client debug, to avoid the timeout messages.
