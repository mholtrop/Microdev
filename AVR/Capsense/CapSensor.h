...
class CapSensor
{
  ...
  public:
  // methods
        CapSensor(volatile uint8_t *sendPort,uint8_t sendPin, volatile uint8_t *receivePort,uint8_t receivePin);
	long sensRaw(uint8_t samples);
	long sens(uint8_t samples);
  ...
  // variables
	...
	volatile IO_REG_TYPE *sPort;
	IO_REG_TYPE rBit;    // receive pin's ports and bitmask
	volatile IO_REG_TYPE *rPort;
  // methods
	int senseOne(void);
};
